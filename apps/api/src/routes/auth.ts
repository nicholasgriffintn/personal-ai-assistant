...
import { githubCallbackSchema, githubLoginSchema, jwtTokenResponseSchema, userSchema } from "./schemas/auth";
import { getUserBySessionId, createOrUpdateGithubUser, createSession, deleteSession } from "../services/user";
...
app.get(
  ...
  describeRoute({
    ...
    responses: {
      200: {
        description: "Given a GitHub OAuth code, this endpoint will authenticate the user and redirect to the original URL",
        content: {
          "application/json": {
            schema: resolver(z.object({})),
          },
...
app.get(
  ...
  async (c: Context) => {
    ...
    const headerSessionId = authHeader && authHeader.startsWith("Bearer ") 
