...
export const createChatCompletionsJsonSchema = z.object({
	...
	messages: z
		.array(
			z.object({
				role: z.enum(["developer", "system", "user", "assistant", "tool"]).openapi({
							...
							image_url: z
								.object({
									url: z.string().url().openapi({
										description: "Either a URL for the image or the base64 encoded data for the image.",
									}),
									detail: z
										.enum(["auto", "low", "high"])
										.optional()
	...
	top_p: z.number().min(0).max(1).default(1).optional().openapi({
		description: "An alternative to sampling with temperature, called nucleus sampling, where the model considers the results of the tokens with top_p probability mass. So 0.1 means only the tokens comprising the top 10% probability mass are considered. Don't specify both top_p and temperature.",
	}),
	...
	stream: z.boolean().optional().openapi({
		description: "If set, partial message deltas will be sent. Tokens will be sent as data-only server-sent events as they become available, with the stream terminated by a data: [DONE].",
	}),
	stop: z.union([z.string(), z.array(z.string())]).optional().openapi({
		description: "Up to 4 sequences where the model will stop generating further tokens. The returned text will not contain these sequences.",
	}),
	...
	frequency_penalty: z.number().min(-2).max(2).default(0).optional().openapi({
		description: "The frequency penalty for the response. Positive values penalize new tokens based on their existing frequency in the text so far, decreasing the model's likelihood to repeat the same line verbatim.",
	}),
	logit_bias: z.record(z.number()).optional().openapi({
		description: "Modify the likelihood of specified tokens appearing in the response. The exact effect will vary per model, but values between -1 and 1 should decrease or increase likelihood of selection; values like -100 or 100 should result in a ban or exclusive selection of the relevant token.",
	}),
	user: z.string().optional().openapi({
		description: "A unique identifier representing the user. If provided, the identifier will be included in the completion log.",
	}),
	metadata: z.record(z.string()).optional().openapi({
		description: "Set of key-value pairs that can be attached to a chat completion for tracking or display purposes. Both keys and values must be strings.",
	}),
	tools: z
		.array(
			z.object({
	...
	reasoning_effort: z.enum(["low", "medium", "high"]).default("medium").optional().openapi({
		description: "Constrains effort on reasoning for reasoning models. Reducing reasoning effort can result in faster responses and fewer tokens used on reasoning in a response. Only supported in certain reasoning models.",
	}),
	...
	response_mode: z.enum(["normal", "concise", "explanatory", "formal"]).optional().openapi({
		description: "The mode of the response. This will affect the style of the response from the model when using default system prompts.",
	}),
