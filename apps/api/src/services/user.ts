 ...
 */
export async function getUserBySessionId(db: D1Database, sessionId: string): Promise<User | null> {
  try {
    const result = await db.prepare(`
      SELECT u.* FROM user u
  ...
  try {
    ...
    if (existingUser) {
      ...
      return {
        ...
        twitter_username: userData.twitterUsername || existingUser.twitter_username,
      ...
      ).bind(userData.email).first() as User | null;
      
      if (userByEmail) {
        await db.prepare(`
          INSERT INTO oauth_account (provider_id, provider_user_id, user_id)
          VALUES ('github', ?, ?)
        `).bind(userData.githubId, userByEmail.id).run();
        ...
        return {
          ...
          twitter_username: userData.twitterUsername || userByEmail.twitter_username,
        ...
        await db.prepare(`
          INSERT INTO oauth_account (provider_id, provider_user_id, user_id)
          VALUES ('github', ?, ?)
        `).bind(userData.githubId, newUser.id).run();
 ...
 */
export async function createSession(db: D1Database, userId: number, expiresInDays = 7): Promise<string> {
  try {
