...
export const ChatMessage: FC<ChatMessageProps> = ({ message, index, setShowMessageReasoning }) => {
					...
					<div className="flex-1">
						{message.reasoning && (
							<div className="mb-2">
								<button
									type="button"
									onClick={() => setShowMessageReasoning(index, !message.reasoning!.collapsed)}
									className="flex items-center text-xs text-zinc-500 dark:text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300"
								>
									{!message.reasoning.collapsed ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
									<span>Reasoning</span>
								</button>
								{!message.reasoning.collapsed && (
									<div className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">{message.reasoning.content}</div>
								)}
							</div>
						)}
						<ReactMarkdown
