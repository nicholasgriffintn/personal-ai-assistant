...
import { replicateWebhookQuerySchema, replicateWebhookJsonSchema } from './schemas/webhooks';
...
app.use('/*', async (context: Context, next: Next) => {
	if (!context.env.WEBHOOK_SECRET) {
		throw new AssistantError('Missing WEBHOOK_SECRET binding', ErrorType.CONFIGURATION_ERROR);
	}
